...
import { getBasesByTypeAndTags, townHallInfo } from '@/data/bases';
...
const BaseTypeFilterComponent = ({ activeType, setActiveType, availableTags, selectedTags, toggleTag, currentSort, setCurrentSort }) => {
  const baseTypes = [
...
const BasesPage = () => {
  ...
  const availableTags = useMemo(() => {
    const bases = getBasesByTypeAndTags(townHallLevel, 'all', [], false);
    const tags = new Set();
  ...
  useEffect(() => {
    const filteredBases = getBasesByTypeAndTags(townHallLevel, activeType, selectedTags, false, currentSort);
    setAllBasesForLevel(filteredBases);
    setCurrentPage(1); 
  }, [townHallLevel, activeType, selectedTags, currentSort]);
  ...
  const pageTitle = `Best TH${townHallLevel} Clash of Clans Layouts`;
  const pageDescription = `Explore top Clash of Clans Town Hall ${townHallLevel} base layouts. Find the best TH${townHallLevel} war, farming, trophy, and hybrid coc bases for your village. Updated for ${new Date().getFullYear()}.`;
  
  const currentTownHallInfo = townHallInfo[parseInt(townHallLevel)];

  return (
    <div className="max-w-screen-xl mx-auto px-2 sm:px-4 lg:px-6 py-6 md:py-8 theme-th">
      <AdPlaceholder />
      <PageHorizontalNav currentPathBase="/bases/" />
      <QuickNav currentLevel={townHallLevel} isBuilderBase={false} />
      <motion.div
      ...
      {currentBases.length > 0 ? (
        ...
        <div className="text-center py-12 bg-card rounded-lg shadow-md border">
          ...
          <p className="text-muted-foreground max-w-md mx-auto">
            Try adjusting your filters or check back later for new Clash of Clans layouts for TH{townHallLevel}.
          </p>
        </div>
      )}
      {currentTownHallInfo && (
        ...
        >
          <h2 className="text-2xl font-bold clash-font text-primary mb-4 flex items-center">
            <Info className="h-6 w-6 mr-3 text-primary/80"/>What's New in Town Hall {townHallLevel}?
          </h2>
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <img 
              src={currentTownHallInfo.imageUrl} 
              alt={`Town Hall ${townHallLevel} Visual`} 
              className="w-full sm:w-1/3 max-w-xs rounded-lg shadow-md border object-cover aspect-video"
            />
            <p className="text-muted-foreground text-sm sm:text-base flex-1">{currentTownHallInfo.new}</p>
          </div>
        </motion.div>
      )}
    </div>
